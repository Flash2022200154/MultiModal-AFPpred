"""
End-to-end pipeline:
ESM-2 embedding + PhysChem(256) -> CrossModalAttention (E->P and P->E, avg) -> ResidualBiLSTM -> Classifier
"""
...
class DualModalPipeline(nn.Module):
    def __init__(
        ...
        super().__init__()
        # ESM-2 embedder (kept outside of nn.Module parameters, used for feature extraction)
        ...
        # Cross-modal attention (two directions)
        self.cma_e2p = CrossModalAttention(
            dim_q=self.esm_dim, dim_kv=physchem_dim, dim_model=dim_model, n_heads=n_heads
        )
        self.cma_p2e = CrossModalAttention(
            dim_q=physchem_dim, dim_kv=self.esm_dim, dim_model=dim_model, n_heads=n_heads
        )

        # Sequence modeling + residual
    ...
    def forward(self, esm_vecs: torch.Tensor, phys_vecs: torch.Tensor) -> torch.Tensor:
        """
        Args:
            esm_vecs: (B, esm_dim)
            phys_vecs: (B, physchem_dim)
        Returns:
            probs: (B,)
        """
        fused_ep = self.cma_e2p(esm_vecs, phys_vecs, phys_vecs)  # (B, dim_model)
        fused_pe = self.cma_p2e(phys_vecs, esm_vecs, esm_vecs)  # (B, dim_model)
        fused = 0.5 * (fused_ep + fused_pe)  # (B, dim_model)

        seq_out = self.seq_block(fused)  # (B, 2*lstm_hidden)
        probs = self.classifier(seq_out)  # (B,)
        return probs

    @torch.no_grad()
    def encode_sequences_to_features(self, sequences: List[str]) -> (torch.Tensor, torch.Tensor):
        """
        Returns:
            esm_vecs: torch.FloatTensor, shape (N, esm_dim), on CPU
            phys_vecs: torch.FloatTensor, shape (N, physchem_dim), on CPU
        """
        # ESM-2 embeddings
        esm_vecs = self.embedder.encode(sequences, batch_size=8, pool="mean", sanitize=True, progress=False)  # (N, H)
        # PhysChem features
        phys_list = [get_physchem_wavelet_multi(s, target_dim=self.physchem_dim) for s in sequences]
        phys_vecs = np.stack(phys_list, axis=0).astype(np.float32)  # (N, 256)

        return torch.from_numpy(esm_vecs.numpy()), torch.from_numpy(phys_vecs)

    @torch.no_grad()
    def predict_on_sequences(self, sequences: List[str]) -> torch.Tensor:
        """
        Convenience end-to-end inference: sequences -> probability scores
        """
        self.eval()
        esm_vecs, phys_vecs = self.encode_sequences_to_features(sequences)
        probs = self.forward(esm_vecs, phys_vecs)
        return probs
...
class MultiModalAFPpred(nn.Module):
    ...
    def predict_on_sequences(self, sequences: List[str]) -> torch.Tensor:
        ...
        probs = self.forward(esm_vecs, phys_vecs, ss_vecs)
        return probs